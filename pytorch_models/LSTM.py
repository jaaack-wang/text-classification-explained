...
About: Simple LSTM model for text classification using pytorch. 
'''
import torch
...
class LSTM(nn.Module):
    ...
    def __init__(self,
        ...
        self.lstm = nn.LSTM(embedding_dim, lstm_hidden_dim, n_layers, 
                          dropout=dropout_rate, bidirectional=self.bidirectional)
        
        lstm_out_dim = lstm_hidden_dim * 2 if self.bidirectional is True else lstm_hidden_dim
    ...
    def encoder(self, embd, seq_len):
        ...
        # Note: there is a "cell" here!
        encoded, (hidden, cell) = self.lstm(embd)
